<section class="bg-gradient-to-b from-amber-50 to-zinc-100 dark:bg-gray-900">
    <div class="grid max-w-screen-xl px-4 py-8 mx-auto lg:gap-8 xl:gap-0 lg:py-16 lg:px-32  sm:grid-cols-12">
        <div class="flex lg:mt-0 sm:col-span-5">
            <img class="col-span-2 row-span-2  h-full object-cover object-top rounded md:rounded-tr-[3rem] md:rounded-bl-[3rem]"
                src="{{ Vite::asset('resources/images/hero.jpg') }}" alt="mockup" data-aos="fade-up"
                data-aos-delay="10">

        </div>
        <div class="mr-auto place-self-center sm:col-span-7 tracking-tight p-8 pb-0 ">
            <h1 class="max-w-2xl mb-4 text-gray-600 text-sm font-extrabold tracking-tight leading-none
             dark:text-white capitalize "
                data-aos="fade-up" data-aos-delay="5">— Nosso Propósito</h1>
            <h2 class="max-w-2xl mb-2 text-2xl font-semibold  tracking-tight leading-none
              capitalize"
                data-aos="fade-up" data-aos-delay="10">1. Missão</h2>

            <div class=" grid  gap-2">
                <div class="" data-aos="fade-up" data-aos-delay="15">
                    <p class=" max-w-2xl   font-light text-gray-700 lg:mb-2 text-sm ">
                        Promover o desenvolvimento sustentável através de pesquisas,
                        assistência técnica, desenvolvimento de ferramentas de monitoria, avaliação dos projectos e
                        mobilização de recursos para projectos socias.
                    </p>

                </div>
                <h2 class="max-w-2xl mb-2 text-2xl font-semibold tracking-tight leading-none
             capitalize"
                data-aos="fade-up" data-aos-delay="10">2. Visão</h2>
                <div class="" data-aos="fade-up" data-aos-delay="15">
                    <p class=" max-w-2xl   font-light text-gray-800 lg:mb-2 text-sm ">
                        Criar uma sociedade dedicada a zelar pêlos direitos das crianças em situação de vulnerabilidade,
                        garantindo acesso a direitos fundamentais, como educação básica de qualidade, cuidados de saúde
                        adequados, protecção contra abusos e exploração, alimentação adequada e oportunidades para um
                        desenvolvimento psicossocial saudável.
                    </p>

                </div>
            </div>
        </div>
    </div>
</section>
